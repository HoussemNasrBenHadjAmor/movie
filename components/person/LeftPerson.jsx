...
const LeftPerson = ({
  ...
  const Info = ({ notShow, text, data, isArray }) => (
    <div className={`${notShow && 'hidden lg:flex lg:flex-col'}`}>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-4">
        ...
        <Info
          text="Gender"
          data={gender === 1 ? 'Female' : gender === 2 ? 'Male' : '-'}
        />

        <Info
          text="Birthday"
          data={birthday ? `${birthday} (${age} years old)` : '-'}
        />
        ...
        <Info
          notShow
          text="Also known As"
          isArray={also_known_as?.length > 1}
          data={also_known_as?.length ? also_known_as : '-'}
        />
      </div>
    </div>
  )
